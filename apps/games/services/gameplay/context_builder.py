...
class ContextBuilder:
    ...
    def _active_mode(self):
        if self.daily_target:
            return self.daily_target.mode
        if self.extra_play:
            return self.extra_play.mode
        if self.challenge:
            return self.challenge.mode
        return None
    ...
    def build(self):
        ...
        attempts = build_attempts(
            self.game, [attempt.guess for attempt in attempts_query], target_item
        )
        ...
        if self.daily_target or self.extra_play:
            ...
            if self.extra_play:
                ...
                global_average = score_service.calculate_global_average_of_averages(
                    exclude_user=True
                )
    ...
    def _get_guess_url(self):
        ...
        mode = self._active_mode()
        if mode:
            return reverse("ajax_guess_mode", args=[self.game.slug, mode.slug])
    ...
    def _get_reveal_hint_url(self):
        ...
        mode = self._active_mode()
        if mode:
            return reverse("ajax_reveal_hint_mode", args=[self.game.slug, mode.slug])

...
def seed_league_modes(apps, schema_editor):
...
def unseed_league_modes(apps, schema_editor):
    Game = apps.get_model("games", "Game")
    GameMode = apps.get_model("games", "GameMode")
    game = _resolve_league_game(Game)
    if game:
        GameMode.objects.filter(game=game, slug__in=["normal", "pasapalabra"]).delete()
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ...
        ("games", "0040_seed_lol_modes"),
    ]

    operations = [
        migrations.RunPython(seed_league_modes, unseed_league_modes),
    ]

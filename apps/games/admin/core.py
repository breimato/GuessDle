...
from apps.games.admin.utils import deep_get
from apps.games.models import ArcCatalog, Game, GameItem, GameMode, ScoringRule
...
class GameAdmin(admin.ModelAdmin):
    ...
    def _process_items(self, request, game, raw_items_list):
        ...
        for raw in raw_items_list:
            ...
            for key, value in game.field_mapping.items():
                if key.upper() in {"NAME", "NOMBRE"}:
                    name_field_in_mapping = value
                    break
            ...
            try:
                ...
                errors_processing.append(f"Error updating/creating item '{name}': {exc}")
        ...
        for game in queryset:
            ...
            if game.json_file:
                ...
                try:
                    ...
                    if not raw_items and isinstance(data, dict) and "results" not in data:
                        messages.warning(
                            request,
                    ...
                    messages.error(
                        request,
                        f"Error leyendo el archivo JSON para '{game.name}': {exc}",
                    )
                    continue
                ...
                try:
                            ...
                            for item_summary in summary_items:
                                ...
                                if detail_url:
                                    try:
                                        ...
                                        messages.warning(
                                            request,
                                            f"Error obteniendo detalle desde {detail_url} para '{game.name}': {exc_detail}",
                                        )
                                else:
                    ...
                    messages.error(
                        request,
                        f"Error obteniendo datos de la API para '{game.name}': {exc}",
                    )
                    continue
                except json.JSONDecodeError as exc:
                    messages.error(
                        request,
                        f"Error decodificando JSON de la API para '{game.name}': {exc}",
                    )
                    continue
        ...
        try:
            with zipfile.ZipFile(zip_file, "r") as zf:
                ...
                if processed_files == 0 and not errors:
                    messages.info(
                        request,
                        "No se procesaron imágenes del ZIP. Asegúrate que los nombres sean 'ID_ITEM.png' y los ítems existan.",
                    )
            ...
            messages.error(
                request,
                f"Ocurrió un error inesperado procesando el archivo ZIP: {exc}",
            )


@admin.register(GameItem, site=admin_site)
...
@admin.register(ScoringRule, site=admin_site)
...
@admin.register(ArcCatalog, site=admin_site)
